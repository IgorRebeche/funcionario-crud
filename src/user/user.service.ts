...
export class UserService {
  ...
  async create(createUserDto: CreateUserDto): Promise<User> {
    ...
    if (!cpf.isValid(cpf.format(createUserDto.cpf)))
    throw new HttpException('Numero de cpf invalido!', HttpStatus.BAD_REQUEST)
    
    if (!Number(createUserDto.age))
    throw new HttpException('Idade precisa ser um numero!', HttpStatus.BAD_REQUEST)
    
    if (Number(createUserDto.age) <= 10)
